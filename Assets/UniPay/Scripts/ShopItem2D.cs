...
namespace UniPay
{
    ...
    public class ShopItem2D : MonoBehaviour
    {
        ...
        public void Refresh()
        {
            ...
            if (expiration && product.type == ProductType.Subscription && isPurchased)
            {
                TimeSpan remaining = IAPManager.controller.GetPurchases()
                    .FirstOrDefault(x => IAPManager.GetFirstProductInOrder(x)?
                    .definition.id == productID)?
                    .Info.PurchasedProductInfo?.ElementAtOrDefault(0)?
                    .subscriptionInfo?.GetRemainingTime() ?? TimeSpan.Zero;
        ...
        public void Purchase()
        {
            ...
            if (askToBuy)
            {
                ...
                if (UIShopFeedback.Instance != null && UIShopFeedback.Instance.purchaseWindow)
                {
    ...
    /// <summary>
    /// Association with costs to a UI label for display purposes.
    /// </summary>
    [System.Serializable]
